...
<Frame style="default.TFrame" columnconfigure-weight="1 1 1 1 1 1 1">
    <Frame style="default.TFrame" grid="columnspan:7;" >
        ...
        <Label text="{{ title }}" pack="side:top; padx:10 10; fill:both; expand:1" anchor="center"/>
    </Frame>
    <Label for="{{d in weekdays}}" text="{{ d }}" grid="{{ {'column': loop_idx, 'row': 2} }}"/>
    <Button for="{{d in dates}}" text="{{ d[2] }}" grid="{{ {'column': loop_idx % 7, 'row': 2 + int(loop_idx / 7)} }}" command="{{_select(d)}}"/>
...
<TopLevel title="TKVue Test" geometry="900x720">
    <Frame style="default.TFrame" pack="fill:both; expand:1;" padding="10">
        <Label text="Hello World !" style="h1.TLabel" pack="padx:25; pady:25"/>
        <Calendar datevariable="{{ selected_date }}" validatecommand="{{_enabled_date}}"/>
        <Label text="{{ 'Selected date: %s' % selected_date}}" />
        <Frame style="default.TFrame" pack="fill: both; expand:1; padx:10; pady:10">
            <Button style="default.TButton" text="Continue" pack="side: right; padx:5"/>
